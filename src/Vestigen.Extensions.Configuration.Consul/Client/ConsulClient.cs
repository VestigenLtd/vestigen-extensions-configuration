...
namespace Vestigen.Extensions.Configuration.Consul.Client
{
    public class ConsulClient : IConsulClient
    {
        ...
        public ConsulClient(IConsulClientAuthenticator authenticator, IConsulSerializer serializer)
        {
        ...
        public IConsulClientAuthenticator Authenticator { get; }

        public IConsulSerializer Serializer { get; }

        public IConsulEndpoints Endpoints { get; }
        ...
        public async Task<string> SendRequest(HttpMethod method, string address, IDictionary<string, string> headers = null, string body = null, CancellationToken? ct = null)
        {
            try
            {
                using (var r = await SendRequestProcessor(method, address, headers, body, ct))
                {
                    ...
                    {
                        throw new ConsulClientRequestException($"Unexpected response, status code {r.StatusCode}", r.StatusCode)
                        {
            ...
            {
                throw new ConsulClientRequestException(ex.Message, default(HttpStatusCode), ex);
            }
        }
        ...
        private Task<HttpResponseMessage> SendRequestProcessor(HttpMethod method, string address, IDictionary<string, string> headers, string body, CancellationToken? ct = null)
        {
            ...
            {
                _httpClient.DefaultRequestHeaders.Add("X-Consul-Token", Authenticator.Token);
            }
